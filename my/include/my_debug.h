...
template <typename A>
static inline int my_print(const A& a) {
  pt(a, true, true);
  return 0;
}
...
constexpr void my_print(Ts&&... args) {
  const int dummy[] = {0, (std::cout << std::forward<Ts>(args), 0)...};
...
static constexpr inline void my_print(const char& fmt, ...) noexcept {}
#endif  // #if CPP >=17
...
#else
// c++11 or greater here
#define TRACE detail::my_trace
#define PRINT detail::my_print
#define SHOW my::detail::my_print
#endif  // #if defined _DEBUG || !defined NDEBUG || defined DEBUG
#endif  // ifndef TRACE
