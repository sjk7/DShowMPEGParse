#pragma once
#ifndef MY_BUFFER_DEFINED_H_MY
#define MY_BUFFER_DEFINED_H_MY
#include <cstdint>
#include <vector>
#ifdef _WIN32
#include <ATLBASE.H>
...
struct slice_method_read {
  ...
  slice_method_read(const slice_method_read& rhs)
      : m_buf(rhs.buf), m_state(rhs.m_state) {}
  ...
  void pos_set(const int newpos) { m_state.read_pos = newpos % m_buf.size(); }
  int pos() const { return m_state.read_pos; }
};
template <typename T>
struct slice_method_write {
  ...
  slice_method_write(const slice_method_read& rhs)
      : m_buf(rhs.buf), m_state(rhs.m_state) {}
  ...
  void pos_set(const int newpos) { m_state.write_pos = newpos % m_buf.size(); }
  int pos() const { return m_state.write_pos; }
};
...
class buffer {
 ...
 public:
  ...
  buffer(size_t reserve_size = MY_DEFAULT_BUFFER_CAPACITY) {
    ASSERT(sizeof(T) ==
           1);  // didn't really build it with other types' sizes in mind.
    ASSERT(reserve_size);
  ...
  void clear() {
    m_vec.clear();
    memset(&m_state, 0, sizeof(impl));
  }

  inline int size() const { return (int)m_vec.size(); }
  // get count of available Ts to read:
  inline int delta() const { return (int)(m_state.written - m_state.read); }
  // get space for writing:
  inline int space() const { return (int)m_vec.size() - delta(); }
...
void buffer_test() {
  ...
  ASSERT(read_slice.ptrs_1.size == 77 && read_slice.ptrs_2.size == 0 &&
         read_slice.ptrs_1.ptr != 0 && read_slice.ptrs_2.ptr == 0);
